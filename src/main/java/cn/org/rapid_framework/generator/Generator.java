...
public class Generator {
	...
	// System.getProperty("generator.removeExtensions",".ftl,.vm");
	private String removeExtensions = System.getProperty(
			"generator.removeExtensions", ".vm");
	...
	private String sourceEncoding = System.getProperty(
			"generator.sourceEncoding", "UTF-8");
	private String outputEncoding = System.getProperty(
			"generator.outputEncoding", "UTF-8");
	...
	public void setSourceEncoding(String sourceEncoding) {
		if (sourceEncoding == null)
			throw new IllegalArgumentException(
					"sourceEncoding must be not null");
	...
	public void setOutputEncoding(String outputEncoding) {
		if (outputEncoding == null)
			throw new IllegalArgumentException(
					"outputEncoding must be not null");
	...
	private void processTemplateRootDirs(Map templateModel, Map filePathModel,
			boolean isDelete, boolean isCommon) throws Exception {
		if (StringHelper.isBlank(getOutRootDir()))
			throw new IllegalStateException(
					"'outRootDir' property must be not null.");
		...
		for (int i = 0; i < this.templateRootDirs.size(); i++) {
			...
			List<Exception> exceptions = scanTemplatesAndProcess(
					templateRootDir, templateModel, filePathModel, isDelete,
					isCommon);
	...
	private List<Exception> scanTemplatesAndProcess(File templateRootDir,
			Map templateModel, Map filePathModel, boolean isDelete,
			boolean isCommon) throws Exception {
		...
		for (int i = 0; i < srcFiles.size(); i++) {
			...
			if (isCommon) { // 只执行DAO.xml模板 **Controller.java ***.ftl
							// （managerui-startup中不需要这些特定的模板）
				if (srcFile.getName().endsWith("DAO.xml")
						|| srcFile.getName().endsWith(".ftl")
						|| (srcFile.getName().endsWith("Controller.java") && !srcFile
								.getName().endsWith("CommonController.java"))) {
					try {
						// 非标准 不生成 controller 、 view、 initsql、commmon
						String parentDir = srcFile.getParent();

						if (parentDir.endsWith("-controller")
								|| parentDir.endsWith("-view")
								|| parentDir.endsWith("-initsql")) {

						} else {
							if (isDelete) {
								new TemplateProcessor().executeDelete(
										templateRootDir, templateModel,
										filePathModel, srcFile);
							} else {
								new TemplateProcessor().executeGenerate(
										templateRootDir, templateModel,
										filePathModel, srcFile);
							}
						}
		...
		private void executeGenerate(File templateRootDir, Map templateModel,
			...
			if (isCopyBinaryFile && FileHelper.isBinaryFile(srcFile)) {
				...
				IOHelper.copyAndClose(new FileInputStream(srcFile),
						new FileOutputStream(new File(getOutRootDir(),
								outputFilepath)));
				return;
			}
