...
public class TableFactory {
    ...
    public static Set<String> IGNORE_TABLES = new HashSet<String>();
	...
	private Table createTable(Connection conn, ResultSet rs) throws SQLException {
		...
		//	System.out.println(rs.getString("TABLE_COMMENT") +"------"+rs.getString("REMARKS"));
...
//            }
    ...
    private List getAllTables(Connection conn) throws SQLException {
        //按新的方式处理
		DatabaseMetaData dbMetaData = conn.getMetaData();
		ResultSet tables = dbMetaData.getTables(conn.getCatalog(), getSchema(), null,
				new String[] { "TABLE" });
	...
	private List getAllTables2(Connection conn) throws SQLException {
        //按新的方式处理
		...
		String tableNames="  and TABLE_NAME in ('user','code','version')";   //只生成这部分表
		System.out.println("select * from TABLES where TABLE_SCHEMA = '" + GeneratorProperties.getRequiredProperty("jdbc.databasename") + "'"+tableNames);
        rs = schemaConn.createStatement().executeQuery("select * from TABLES where TABLE_SCHEMA = '" + GeneratorProperties.getRequiredProperty("jdbc.databasename") + "'"+tableNames);
